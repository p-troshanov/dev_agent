...
async def run_task(task_id: int, initial_prompt: str, is_continue: bool = False, user_id: int = None):
    ...
    log_task_action(task_id, "system", "Продолжение работы..." if is_continue else "Инициализация задачи...", "")
    ...
    try:
        ...
        if not row:
            log_task_action(task_id, "system", "Задача не найдена или удалена.", "")
        ...
        if not agent_name: agent_name = "Основной"
        ...
        # Получаем данные проекта (если есть)
        project_context = ""
        if project_id:
            with get_db() as conn:
                with conn.cursor() as c:
        ...
        # Автоматическое сканирование для пошаговых задач (при первом запуске, не continue)
        if task_type == "step_by_step" and not is_continue:
            try:
                scan_res = await execute_fs_code_tool(
                    f_name="scan_project",
                    ...
                    user_id=user_id,
                    task_id=task_id,
                    tool_call_id="auto_scan",
                    tool_settings={}
                )
                scan_text = scan_res.get("result_str", "")
                if scan_text:
                    project_context += f"\n--- Автоматическое сканирование структуры проекта ---\n{scan_text}\n"
                    log_task_action(task_id, "system", f"Автоматическое сканирование проекта:\n{scan_text}", "")
            except Exception as e:
                print(f"[RUNNER] Ошибка авто-сканирования: {e}", flush=True)
        ...
        # Сборка финального промпта (sys_prompt идет последним)
        full_prompt_parts = []
