...
async def run_task(task_id: int, initial_prompt: str, is_continue: bool = False, user_id: int = None):
    ...
    log_task_action(task_id, "system", "Продолжение задачи..." if is_continue else "Инициализация задачи...", "Система")
    ...
    if not row:
        log_task_action(task_id, "system", "Ошибка: Агент для задачи не найден.", "Система")
