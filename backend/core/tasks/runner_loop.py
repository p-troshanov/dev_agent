...
async def run_task_loop(
    ...
    print(f"\n[RUNNER LOOP] 🚀 Старт цикла задачи ID: {task_id} (Агент: {agent_name}, Модель: {agent_model})", flush=True)
    
    for i in range(MAX_ITERATIONS):
        print(f"\n[RUNNER LOOP] --- Итерация {i+1}/{MAX_ITERATIONS} ---", flush=True)
        
        if is_task_cancelled(task_id):
            print(f"[RUNNER LOOP] 🛑 Задача {task_id} отменена пользователем.", flush=True)
            log_task_action(task_id, "system", "Задача отменена.", "Система")
        ...
        for idx, m in enumerate(messages):
            ...
            if is_target_tool and len(content_str) > 1500:
                ...
                if is_old:
                    m["content"] = "[Результат скрыт для экономии контекста]"
        ...
        if not response or not response.choices:
            print(f"[RUNNER LOOP] ❌ Ошибка LLM (Rate Limit или API).", flush=True)
            log_task_action(task_id, "assistant", "Не удалось получить ответ от LLM (Rate Limit или ошибка API).", agent_name)
            update_task_status(task_id, 'failed')
            break
        ...
        if len(recent_actions) == 3 and recent_actions[0] == recent_actions[1] == recent_actions[2]:
            print(f"[RUNNER LOOP] ⚠️ Обнаружено зацикливание LLM! Ожидание пользователя.", flush=True)
            update_task_status(task_id, 'waiting_user')
            await state.broadcast_ws({"type": "TASK_UPDATED", "task_id": task_id}, user_id)
            
            warning_msg = "Зафиксировано зацикливание действий (однотипные запросы). Продолжить?"
            ...
            if not approved:
                print(f"[RUNNER LOOP] 🛑 Цикл прерван пользователем.", flush=True)
                log_task_action(task_id, "system", "Выполнение остановлено пользователем.", "Система")
                update_task_status(task_id, 'failed')
                await state.broadcast_ws({"type": "TASK_UPDATED", "task_id": task_id}, user_id)
                break
            else:
                reminder = f"[{datetime.now().strftime('%H:%M:%S')}] [Внимание: Зафиксировано повторение одних и тех же действий. Проанализируйте предыдущие ошибки и измените стратегию!]"
                log_task_action(task_id, "system", reminder, "Система")
                messages.append({"role": "assistant", "content": (message.content or "Цикл прерван...")})
        ...
        if hasattr(message, 'tool_calls') and message.tool_calls:
            print(f"[RUNNER LOOP] 🛠 LLM вызвала инструменты: {[tc.function.name for tc in message.tool_calls]}", flush=True)
            ...
            async def process_single_tc(tc):
                if abort_subsequent_tools[0] or is_task_cancelled(task_id):
                    return tc, "[Прервано из-за предыдущей критической ошибки].", False
                ...
                print(f"[RUNNER LOOP] ➡️ Выполнение {f_name} с аргументами: {args}", flush=True)
                ...
                print(f"[RUNNER LOOP] ⬅️ Результат {f_name}: {res_str[:200]}{'...' if len(res_str) > 200 else ''}", flush=True)
            ...
            for tc in message.tool_calls:
                ...
                if is_finish:
                    ...
                    if consecutive_tool_errors >= 2:
                        error_abort_msg = f"[Система] Агент застрял на ошибке инструмента '{tc_obj.function.name}'. Задача прервана для избежания траты токенов."
                        log_task_action(task_id, "system", error_abort_msg, "Система")
                        update_task_status(task_id, 'failed')
                        task_failed_due_to_error_loop[0] = True
                        break
                        
                # ИСПРАВЛЕНИЕ ЗДЕСЬ: Прерываем только цикл обхода ИНСТРУМЕНТОВ
                if abort_subsequent_tools[0]:
                    break
                    
            # ИСПРАВЛЕНИЕ ЗДЕСЬ: abort_subsequent_tools больше не прерывает основной цикл LLM!
            ...
            if text_reply:
                print(f"[RUNNER LOOP] 💬 LLM ответила текстом: {text_reply[:150]}...", flush=True)
                log_task_action(task_id, "assistant", text_reply, agent_name)
                ts_prefix = f"[{datetime.now().strftime('%H:%M:%S')}] "
                messages.append({"role": "assistant", "content": f"{ts_prefix}{text_reply}"})
                
            reminder = f"[{datetime.now().strftime('%H:%M:%S')}] [Система] Напоминание: Вы не вызвали ни одного инструмента! Для ответа используйте 'message_user'!"
            log_task_action(task_id, "system", reminder, "Система")
            messages.append({"role": "user", "content": reminder})
            continue
    else:
    ...
    print(f"[RUNNER LOOP] 🏁 Цикл задачи {task_id} завершен.", flush=True)
    await state.broadcast_ws({"type": "TASK_UPDATED", "task_id": task_id}, user_id)
